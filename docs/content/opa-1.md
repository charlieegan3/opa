...
Here, these changes are described, along with the presentation of tools to make new and existing policies compatible with OPA v1.0.
...
## Running OPA in v1.0 compatibility mode

OPA can be run in v1.0 compatibility mode by using the `--v1-compatible` flag. When this mode is enabled, the current release of OPA will behave as OPA v1.0 will eventually behave by default when it comes to the changes described [here](#whats-changing-in-opa-10).
