...
their Rego is already Rego v1 compatible. See [Upgrading Rego](#upgrading-rego)
below for information on how to upgrade Rego policies to Rego v1.
...
once, you will need to upgrade your Rego policies to Rego v1. This section
outlines the changes in Rego v1. See

### The `future.keywords` imports

The `in`, `every`, `if` and `contains` keywords have been introduced over time,
and in Rego v0 required an opt-in to prevent them from breaking policies that
...
In OPA v1.0 the `in`, `every`, `if` and `contains` keywords are be part of the
...
In Rego v0, there is semantic ambiguity between rules like `a.b {true}` and
...
the semantics of rules do not change between v0 and v1 Rego. The table below
illustrates why `if` is required.

| rule              | output in v0             | output in v1.0           |
| ----------------- | ------------------------ | ------------------------ |
...
outcome of a rule like `p.a { true }` would change between v0 and v1.0
...
The below table gives examples of v0 valid Rego syntax which are
...
The Rego compiler supports [strict mode](../policy-language/#strict-mode) which
enforces additional constraints and safety checks during compilation. One of
these checks is to prohibit duplicate imports where one import shadows another.
...
The Rego compiler supports [strict mode](../policy-language/#strict-mode), which
enforces additional constraints and safety checks during compilation. One of
these checks is to ensure that `input` and `data` are reserved keywords and may
not be used as names for rules and variable assignments.
...
of erasing information under these, resulting in incorrect policy decisions. In
...
The Rego compiler supports [strict mode](../policy-language/#strict-mode), which
enforces additional constraints and safety checks during compilation. One of
these checks is to prohibit use of deprecated built-in functions. In OPA v1.0,
...
Users with v0 Rego projects are encouraged to follow the below process to
...
[detailed blog post](https://www.styra.com/blog/renovating-rego/) - please see
there for more detail, especially if you have a very old Rego codebase which you
are upgrading.
